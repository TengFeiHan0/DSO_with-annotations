...
namespace ldso {
    ...
    /*
    * @ function: 程序入口 包括数据预处理和初始化部分
    * 
    * @ param: 	image		标定后的辐照度和曝光时间
    * @ param:			id			帧的序号
    * 
    * @ see        paper:direct sparse odometry  3.1
    */
    void FullSystem::addActiveFrame(ImageAndExposure *image, int id) {
        ...
        if (!initialized) {
            ...
            //!步骤5 跟踪新来的frameHessian,判断跟踪的状态             init finished, do tracking
                //!步骤5.1 =========================== SWAP tracking reference?. =========================
            ...
            //!步骤5.2 假设不同的旋转平移去估计运动状态  see paper DSO 3.1 step1
    ...
    * @	param: needKF     是不是关键帧		
    * 
    * @ see        https://www.cnblogs.com/JingeTU/p/8329780.html
    */
    ...
    Vec4 FullSystem::trackNewCoarse(shared_ptr<FrameHessian> fh) {
        ...
        if (allFrameHistory.size() == 2)
            ...
            SE3 fh_2_slast = slast_2_sprelast;// 运动一致假设assumed to be the same as fh_2_slast.
    ...
    void FullSystem::makeKeyFrame(shared_ptr<FrameHessian> fh) {
        ...
        auto refFrame = frames.back();//设置参考帧
        ...
        /*
        *@function  步骤2， 标记需要被边缘化的关键帧
        * @condition1 共视点的百分比
        * @condition2 距离函数
        * @condition3 保留最近的两个关键帧
        * 
        * @see the original paper(DSO) #3.1 step3
        */
        ...
        //!步骤4 ======= add new residuals for old points =========================
        //插入新的帧之后，肯定就会产生新的残差
        ...
        //!步骤5 ========= Activate Points (& flag for marginalization). =========================
        activatePointsMT();
        ef->makeIDX();
        ...
        //!步骤7 =========================== Figure Out if INITIALIZATION FAILED =========================
        ...
        //!步骤9 =========================== (Activate-)Marginalize Points =========================
        ...
        //!步骤10 =========================== add new Immature points & new residuals =========================
        makeNewTraces(fh, 0);
    ...
    float FullSystem::optimize(int mnumOptIts) {
        ...
        //!1 加入未被边缘化的残差 get statistics and active residuals.
        ...
        //!2 线性化activeResiduals的残差, 计算边缘化的能量值 (然而这里都设成0了)
    ...
    void FullSystem::activatePointsMT() {
            ...
            for (size_t i = 0; i < host->frame->features.size(); i++) {
                ...
                if (feat->status == Feature::FeatureStatus::IMMATURE && feat->ip) {
                    ...
                    if (!std::isfinite(ph->idepth_max) || ph->lastTraceStatus == IPS_OUTLIER) {
                      ...
                      //!激活条件，下列这些种类的点中，质量、深度、搜索范围大小满足要求的点激活
        ...
        //优化上一步挑出来的未成熟点, 进行逆深度优化, 并得到pointhessian
        if (multiThreading) {
            threadReduce.reduce(
...
*/
    void FullSystem::initializeFromInitializer(shared_ptr<FrameHessian> newFrame) {
        ...
        //!4 生成未成熟点Create features in the first frame.
        for (size_t i = 0; i < size_t(coarseInitializer->numPoints[0]); i++) {
            
            //!4.1 如果提取的点比较少, 不执行; 提取的多, 则随机干掉
...
*toRemove,int min,int max, Vec10 *stats,int tid) {

        for (int k = min;k < max;k++) {
            shared_ptr<PointFrameResidual> r = activeResiduals[k];
            (*stats)[0] += r->linearize(Hcalib->mpCH); // 线性化得到能量和
...
// applies step to linearization point.
    ...
    void FullSystem::setNewFrameEnergyTH() {
        ...
        int nthIdx = setting_frameEnergyTHN * allResVec.size();//70%
