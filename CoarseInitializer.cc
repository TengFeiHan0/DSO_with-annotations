...
namespace ldso {
    ...
    CoarseInitializer::~CoarseInitializer() {
        ...
        delete[] JbBuffer;//别忘了加上“[]”
        delete[] JbBuffer_new;
    } 
    /*
    *@ brief  跟踪新的帧
    * 
    * @param newFrameHessian  新的帧
    * 
    * @see https://www.cnblogs.com/JingeTU/p/8329780.html 2.1
    */
    bool CoarseInitializer::trackFrame(shared_ptr<FrameHessian> newFrameHessian) {

        newFrame = newFrameHessian;
        int maxIterations[] = {5, 5, 10, 30, 50};//迭代次数

        alphaK = 2.5 * 2.5;//?f**k, 为什么是这样的，应该是下面判断平移的参数
        alphaW = 150 * 150;
        regWeight = 0.8;//?f**k怎么这么多奇怪的参数
        couplingWeight = 1;
       //!步骤1，判断之前第一帧是否收敛，如果不收敛，则重置
        if (!snapped) {
        ...
        for (int lvl = pyrLevelsUsed - 1; lvl >= 0; lvl--) {
            if (lvl < pyrLevelsUsed - 1)
                propagateDown(lvl + 1);//向下计算

            Mat88f H, Hsc;
            Vec8f b, bsc;
            resetPoints(lvl);//2.1 对顶层初始化
             //2.2 计算能量，hessian矩阵等信息
    ...
    Vec3f CoarseInitializer::calcResAndGS(
        ...
        Eigen::Vector3f *colorRef = firstFrame->dIp[lvl];//参考帧（第一帧）的图像和梯度
        ...
        for (int i = 0; i < npts; i++) {
            ...
            for (int idx = 0; idx < patternNum; idx++) {
                ...
                //得到此时的图像梯度(输入3维，输出3维像素值 + x方向梯度 + y方向梯度)
            ...
            if (!isGood || energy > point->outlierTH * 20) {
                E.updateSingle((float) (point->energy[0]));
                point->isGood_new = false;
                point->energy_new = point->energy;//还是用之前的，上一帧的给新一帧的
                continue;
            }
    ...
    void CoarseInitializer::propagateDown(int srcLvl) {
        ...
        for (int i = 0; i < nptst; i++) {
            ...
            if (!point->isGood) {
                //iR 变量相当于是逆深度的真值，
                //在优化的过程中，使用这个值计算逆深度误差，效果是幅面中的逆深度平滑。
    ...
    /*
    *@brief 设置第一帧
    *@param  HCalib 相机内参（构建金字塔的时候用到）
    *@param  newFrameHessian 第一帧
    *@see https://blog.csdn.net/xxxlinttp/article/details/89379785   #1
   */
    void CoarseInitializer::setFirst(shared_ptr<CalibHessian> HCalib, shared_ptr<FrameHessian> newFrameHessian) {
        //!步骤1,计算金字塔每层的内参
        makeK(HCalib);
        firstFrame = newFrameHessian;
        //! 步骤2，选择局部梯度大像素点（通过划分网格使点分布均匀）
        PixelSelector sel(w[0], h[0]);

        float *statusMap = new float[w[0] * h[0]];
        bool *statusMapB = new bool[w[0] * h[0]];

        float densities[] = {0.03, 0.05, 0.15, 0.5, 1};//点密度
        //!步骤3，对金字塔选择像素点,并对所有的点进行初始化
        for (int lvl = 0; lvl < pyrLevelsUsed; lvl++) {
            sel.currentPotential = 3;//网格大小
            int npts;
            //!步骤3.1 每一层选择点，得到点的数量
            if (lvl == 0) {
            ...
            //!3.2 set idepth map to initially 1 everywhere.
            int wl = w[lvl], hl = h[lvl];
            Pnt *pl = points[lvl];
            int nl = 0;
        ...
        thisToNext = SE3();
        snapped = false;//是否收敛
        frameID = snappedAt = 0;//收敛在第几帧

        for (int i = 0; i < pyrLevelsUsed; i++)
            dGrads[i].setZero();

    }
    //用周围临近点逆深度的平均值去设置点的逆深度
    void CoarseInitializer::resetPoints(int lvl) {
        Pnt *pts = points[lvl];
        int npts = numPoints[lvl];
        for (int i = 0; i < npts; i++) {
            pts[i].energy.setZero();//初始化
            ...
            if (lvl == pyrLevelsUsed - 1 && !pts[i].isGood) {
                ...
                if (sn > 0) {
                    pts[i].isGood = true;//初始化为临近点逆深度的平均值
    ...
    void CoarseInitializer::makeNN() {
        ...
        for (int lvl = 0; lvl < pyrLevelsUsed; lvl++) {
            ...
            for (int i = 0; i < npts; i++) {
                ...
                //* 给每个点的neighbours赋值
                for (int k = 0; k < nn; k++) {
                    pts[i].neighbours[myidx] = ret_index[k];//索引
