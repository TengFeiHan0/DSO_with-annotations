...
namespace ldso {
   /*
   *@function: 用sse加快计算描述子的距离
   *@see:SSE手册 https://software.intel.com/sites/landingpage/IntrinsicsGuide/ 
   ...
   */
    int FeatureMatcher::DescriptorDistance(const unsigned char *desc1, const unsigned char *desc2) {
        ...
        for (int i = 0; i < 8; i++, pa++, pb++) {
            ...
            //SSE速度贼拉快，牛逼
            //*_mm_popcnt_u64功能是Count the number of bits set to 1 in unsigned 64-bit integer a, 
            //*and return that count in dst.
            //!更多内容参见https://software.intel.com/sites/landingpage/IntrinsicsGuide/
#ifdef __SSE2__
            dist += _mm_popcnt_u64(v);  // SSE is so easy
#else
            //!以前的方法真傻比，另外这个函数和ORBSLAM里面使用的是一样的
    ...
    int FeatureMatcher::SearchBruteForce(shared_ptr<Frame> frame1, shared_ptr<Frame> frame2,
        ...
        for (size_t i = 0; i < frame1->features.size(); i++) {
            ...
            for (size_t j = 0; j < frame2->features.size(); j++) {
                ...
                if (dist < min_dist) {
                    min_dist = dist;//得到最小距离
                    min_dist_index = j;//最小距离的index
                }
            }
    ...
    /*
    *@function: 通过词包，对关键帧的特征点进行跟踪 
    *@see 这部分直接参照ORBSLAM中的注释 
    *https://gitee.com/paopaoslam/ORB-SLAM2/blob/master/src/ORBmatcher.cpp 
    */
